...
- 用户自定信号处理函数，回跑到内核去执行吗？内核看不到用户数据呀？
  ...
  ![img](https://img-blog.csdn.net/20130911191240031?watermark/2/text/aHR0cDovL2Jsb2cuY3Nkbi5uZXQvRExVVEJydWNlWmhhbmc=/font/5a6L5L2T/fontsize/400/fill/I0JBQkFCMA==/dissolve/70/gravity/SouthEast)

![image-20201103220502445](/Users/wangchuanyi/doc/weekly_read/book/CSAPP/image//image-20201103220502445.png)
...
> 一个函数在执行，中断 执行操作过程中 
>
>  信号也在执行。从时间断上看是同时在执行 [22222].   [2222]
>
> 都会操作全局数据，不安全的。
>
...
- 虚拟内存 存储在磁盘上，不应该存储在内存吗？
  ...
  这样体现了 内存是磁盘的缓存的概念，都是一家人。
  
  
  
  
  
  ![image-20201103221855084](/Users/wangchuanyi/doc/weekly_read/book/CSAPP/image//image-20201103221855084.png)å
...
二. 缓存在文件系统中作用（slabtop查看）

1. 虚拟文件系统（对不同文件系统的抽象）

2. 文件系统（参考第一期性能优化 ）

   目录项（完全是内存缓存，关机后会100%丢失，但是目录是个文件，存储在磁盘中 ）
